    ...
    return (
        <div className="">      
            <div className=" grid grid-cols-3 gap-2">
                ...
                <Card className="col-span-full mt-0" title="Trending Products">
                    <Swiper
                        spaceBetween={10}
                        navigation={true}
                        className="w-full"
                        ...
                        { products && products.map( data => (
                            <SwiperSlide className="shadow-product rounded-md m-2 h-28 mobile:w-5/6">
                                <div className="flex h-full">
                                    <div className="w-1/3 h-full bg-cover bg-center rounded-tl-md rounded-bl-md" style = {{background: `url("${data.photo[0].url}")`}}/>
                                    <div className="w-2/3 text-center font-bold text-Black my-auto">
                                        <Link to= {`/marketplace/products/${data.slug}`} className="font-bold text-Black">
                                            { data.name }
                                        </Link>
                                    </div>
                                </div>
